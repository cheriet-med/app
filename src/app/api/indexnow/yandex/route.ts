// app/api/indexnow/yandex/route.ts
...
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (response.ok) {
      return NextResponse.json({
        success: true,
        message: `Successfully submitted ${urlList.length} URL(s) to Yandex IndexNow`,
        data: responseData,
      });
    } else {
      return NextResponse.json({
        success: false,
        error: `Yandex IndexNow API returned ${response.status}: ${response.statusText}`,
        data: responseData,
      }, { status: response.status });
    }
  } catch (error) {
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to submit to Yandex IndexNow', 
